...
class RetrievalEngine:
    ...
    3. Score every candidate with ``RankerService`` (MMoE heavy ranker).
       Repos without a real Qdrant embedding (e.g. trending-only repos) are
       bypassed from the MMoE and scored via cosine fallback instead.
    ...
    def _rank_candidates(
        ...
        - ``final_score``   — normalised [0, 1] weighted value-function output
        - ``predictions``   — raw per-task probabilities (p_ctr, p_save, …)
        - ``score_source``  — "mmoe" or "cosine_fallback" (if ranker unavailable)
        """
        if not candidates:
            return []
